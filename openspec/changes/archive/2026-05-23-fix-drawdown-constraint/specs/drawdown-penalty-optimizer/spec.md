...
- **THEN** the function SHALL return 0.0 (no drawdown possible with only two data points where both are from cummax)
...
- **AND** `result.success` SHALL be `true` (optimization completed but constraint soft)
